...
class EscalonadorCAV(ABC):
    ...
    def calcular_turnaround(self):
        """Calcula o tempo de turnaround médio das tarefas já concluídas"""
        concluidas = [tarefa for tarefa in self.tarefas if tarefa.tempo_final is not None]
        if not concluidas:
            return None
        soma = sum(tarefa.tempo_final - tarefa.tempo_chegada for tarefa in concluidas)
        return soma / len(concluidas)
...
class EscalonadorRoundRobin(EscalonadorCAV):
    def __init__(self, quantum, sobrecarga = 1):
        super().__init__()
        self.quantum = quantum
        self.sobrecarga = sobrecarga  #sobrecarga como 
            ...
            while fila:
                ...
                if tarefa.tempo_restante > 0:
                    ...
                    if tarefa.tempo_restante > 0:
                        ...
                        self.registrar_sobrecarga(self.sobrecarga)  # 1 segundo de sobrecarga por tarefa
                        self.tempo_atual += self.sobrecarga
    ...
    def escalonar(self):
        ...
        if (len(self.tarefas) > 0):
            ...
            while (fila):
                ...
                print(f"[{self.tempo_atual}s] Executando tarefa {tarefa.nome} de {tarefa.duracao} (chegada: {tarefa.tempo_chegada}s)")
...
class CAV:
    ...
    def calcular_turnaround(self, escalonador):
        return escalonador.calcular_turnaround()
...
def criar_tarefas():
    tarefas = [
        ...
        # Testes fixos
        # TarefaCAV("Detecção de Obstáculo", duracao=4, prioridade=1, tempo_chegada=0, possivelmente_catastrofico=True),
        # TarefaCAV("Planejamento de Rota", duracao=2, prioridade=1, tempo_chegada=2, possivelmente_catastrofico=False),
        # TarefaCAV("Manutenção de Velocidade", duracao=1, prioridade=1, tempo_chegada=4, possivelmente_catastrofico=False),
        # TarefaCAV("Comunicando com Infraestrutura", duracao=3, prioridade=1, tempo_chegada=6, possivelmente_catastrofico=False),
    ]
    return tarefas

# Exemplo de uso
if __name__ == "__main__":
    # Criar algumas tarefas fictícias e printa
    tarefas = criar_tarefas()
    for t in tarefas:
        print(f'Nome: {t.nome}, duração: {t.duracao}, tempo de chegada: {t.tempo_chegada}')
    ...
    print(f"O valor do turnaround no FIFO é de {simulador_fifo.calcular_turnaround(escalonador_fifo)} segundos.\n")

    # Criar um escalonador Round Robin com quantum de 2 segundos
    print("\nSimulando CAV com Round Robin:\n")
    escalonador_rr = EscalonadorRoundRobin(quantum=2)
    for t in tarefas:
        escalonador_rr.adicionar_tarefa(t)

    simulador_rr = CAV(id=1)
    simulador_rr.executar_tarefas(escalonador_rr)
    simulador_rr.calcular_turnaround(escalonador_rr)
    print(f"O valor do turnaround no RR é de {simulador_rr.calcular_turnaround(escalonador_rr)} segundos.\n")
    ...
    simulador_prio.calcular_turnaround(escalonador_prio)
    print(f"O valor do turnaround no Prioridade é de {simulador_prio.calcular_turnaround(escalonador_prio)} segundos.\n")

    # Criar um escalonador SJF
    print("\nSimulando CAV com Escalonamento SJF:\n")
    ...
    simulador_sjf.calcular_turnaround(escalonador_sjf)
    print(f"O valor do turnaround no SJF é de {simulador_sjf.calcular_turnaround(escalonador_sjf)} segundos.\n")